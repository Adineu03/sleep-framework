...
\paragraph{Proposed:} SLEEP's safety mechanisms are designed for repeated-cycle continual learning, not for single-cycle one-shot benchmarks. When we evaluate in their intended regime, SLEEP's per-cost efficiency overtakes naive LoRA.
...
\paragraph{Multi-cycle interpretation:} This multi-cycle interpretation supersedes the single-cycle one. Single-cycle benchmarks systematically disadvantage architectures whose value emerges across cycles. The naive-LoRA-wins picture from Section~\ref{sec:results_pareto} is true for one-shot evaluation but does not generalize. We frame this as a methodological note for the field: continual-learning systems should not be evaluated by their per-cost efficiency on a single training pass.
