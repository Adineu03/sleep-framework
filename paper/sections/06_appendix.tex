...
\paragraph{A.6 (Consolidation placement must come from the transformer's causal structure, not biological analogy):} The original formalization mapped ``neocortex'' to the top third of layers and adapted their attention V/O projections, by analogy to hippocampal--neocortical anatomy. This is the largest amendment: causal-tracing evidence places factual associations in mid-stack MLP modules, and the five-arm isolation of Section~\ref{sec:results_localisation} confirms the analogy-driven placement was the leading cause of the consolidation failure (with single-wording replay and clip calibration as the second and third). Required correction: $W_{\mathrm{cons}}$ adapts mid-stack MLP up/down projections, decoupled from the KV-injection placement, which retains its independently verified top-third-attention requirement; replay must carry surface diversity; $\delta_{\max}$ at 7--8B scale belongs near $0.1$ during sleep-phase training, not $0.01$. Sections~\ref{sec:method_v2} and \ref{sec:results_localisation}--\ref{sec:results_matrix} specify and validate the amended design.
